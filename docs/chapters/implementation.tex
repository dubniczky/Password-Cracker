...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        \end{tabular}
    \end{tabular}
    \caption{Három bináris függvény eredménye.}
\end{table}

A példákból látszik hogy nem minden bináris függvény esetén kapunk arányos mennyiségű igaz és hamis értéket. Ez természetesen azt okozza hogy n lépés esetén az eredmény konvergálni fog a magasabb esélyű értékhez.
...
\lstset{caption={A kulcs lokális memóriába másolása az eszközön.}, label=src:cpp}
\begin{lstlisting}[language={C++}]
...
\lstset{caption={A kulcs néggyel egészen osztható részének az uint tömbbe másolása.}, label=src:cpp}
\begin{lstlisting}[language={C++}]
//Copy whole uints
qua = length / 4;
mod = length % 4;
...
\lstset{caption={A kulcs néggyel nem osztható részének az uint tömbbe másolása.}, label=src:cpp}
\begin{lstlisting}[language={C++}]
...
A grafikonról látható, hogy a legtöbbször használt jelszavak hossza a mod 4 = 0 kategóriába esik. Ennek egy oka lehet, hogy sok weboldal megkövetel legalább 8 karaktert, és páran maximum 16 karaktert jelszónak.
...
\lstset{caption={Az üzenetsor tömörítése a kezdeti kulcsból.}, label=src:compress}
\begin{lstlisting}[language={C++}]
//Prepare compression
A = H0;
B = H1;
